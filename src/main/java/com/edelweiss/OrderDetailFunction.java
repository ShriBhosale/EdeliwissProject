...
public class OrderDetailFunction {
	...
	private String step;
	private LatestLoginModel latestLoginModel;
	...
	public void executionBefore() throws IOException {
		...
		apacheCodeObj = new ApacheCode(OrderDetailTestLauncher.folderPath[0]);
	...
	public void executionWithKeyword(String referenceNo, String userId, String pwd, String yob, String startNo,
		...
		if (execute.equalsIgnoreCase("Yes")) {
			...
			OrderDetailLoginModel loginModelObj = new OrderDetailLoginModel(referenceNo, userId, pwd, yob, startNo, endNo, module, execute);
			Reporter.log("Login Data ====> " + loginModelObj.toString(), true);
			 latestLoginModel=new LatestLoginModel(userId,pwd,yob,"OrderDetail");
			...
			while (csvMasterTestModelIterator.hasNext()) {
				...
				case "login":
					...
					loginPageObj.loginExecution("normal", latestLoginModel,driver);
					break;
	...
	public void terminateExecution(String module, WebDriver driver) throws InterruptedException, IOException {
		...
		reporter.captureScreen(driver, OrderDetailTestLauncher.folderPath[1], "LastScreenshot", 1);
		if (driver != null) {
			if (!module.equalsIgnoreCase("orderdetail")) {
				
				reporter.reporter(driver, module, OrderDetailTestLauncher.folderPath);
	...
	public void afterSuite() throws IOException {
		...
		Reporter.log("Folder Path ====> " + OrderDetailTestLauncher.folderPath[0], true);
		apacheCodeObj.outputExcelFileClose(OrderDetailTestLauncher.folderPath[0]);
